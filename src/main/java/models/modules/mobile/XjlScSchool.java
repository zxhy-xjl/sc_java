...
@Entity
@Table(name = "xjl_sc_school")
public class XjlScSchool extends GenericModel {
	 ...
	 * @return 返回学校信息
	 */
	public static XjlScSchool getSchoolBySchoolId(Long schoolId,String schoolDoMain) {
		int pageIndex = 1;
        int pageSize = 100;
        ...
        if(returnMap!=null&&returnMap.get("data")!=null){
        	if("1".equals(returnMap.get("total").toString())){
            	List<XjlScSchool> retData = (List<XjlScSchool>)returnMap.get("data");
            	if(retData.size()>0){
        		...
        		Logger.error("==========ERROR==========查出的学校信息不唯一，共有"+returnMap.get("total")+"条",XjlScSchool.class);
        		return null;
        	}
        }else{
        	return null;
	...
	public static Map queryXjlDwSchoolListByPage(Map<String, String> condition,
			...
			List<XjlScSchool> data =  new ArrayList<XjlScSchool>();
			XjlScSchool xjlDwSchool;
			for(Object[]m :retData){
				xjlDwSchool = new XjlScSchool();
	...
	public static List<XjlScSchool> querySchoolList(Map<String, String> condition,
			int pageIndex, int pageSize){
		String sql = "select * from xjl_sc_school where status='0AA' order by create_time desc";
		SQLResult ret = ModelUtils.createSQLResult(condition, sql);
		List<XjlScSchool> data = ModelUtils.queryData(pageIndex, pageSize, ret,XjlScSchool.class);
		return data;
	}
